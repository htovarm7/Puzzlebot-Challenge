#!/usr/bin/env python3
"""Show the line_detector debug image in an OpenCV window.

Requires a display (local or SSH with X11 forwarding: ssh -X).
Subscribes to /vision/line (sensor_msgs/Image).
"""
...
class LineViewerNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info('LineViewer ready, waiting for /vision/line ...')
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().error(
            ...
            'Use "ssh -X" for X11 forwarding or disable line_viewer.'
        )
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
